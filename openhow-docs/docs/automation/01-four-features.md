---
slug: automation/four-features
title: 4가지 기능
nav: 4가지 기능
description: 'CLAUDE.md와 업무 문서의 여러 목차를 실제 작업에서 지키게 만드는 자동화 장치 commands, agents, skills, hooks를 입문자 관점에서 정리한다.'
thumbnail: /__content__/images/img-nb-sec-automation.png
hook: '자동화를 가능하게 하는 Claude Code의 4가지 요소예요.'
status: 출시
tags: [자동화, commands, agents, skills, hooks, context]
access: public
---

## 핵심 요약

**자동화는 AI가 매번 알아서 모든 목차와 업무 문서를 챙기길 기대하는 게 아니라, 어떤 일을 시킬 때 어떤 목차를 읽고, 어떤 순서로 처리하고, 어떤 기준으로 확인할지 실행 흐름으로 묶어두는 거예요.**

앞에서 프로젝트 안에 `CLAUDE.md`와 업무 문서를 만들었죠. 그게 시작이에요. 그런데 문서가 많아지면 새로운 문제가 생겨요.
...
문서가 많아질수록 기준을 작업 흐름 안에 직접 배치해야 해요. 어떤 작업에서 어떤 목차를 읽을지, 어떤 업무 문서를 참고할지, 어떤 순서로 처리할지 정해둬야 해요.

자동화는 이걸 실행 가능한 흐름으로 묶는 거예요.

> 이 작업은 이 목차를 읽고, 이 업무 문서를 참고하고, 이 순서로 처리하고, 마지막에는 이 기준으로 확인한다.

이런 실행 흐름을 워크플로우, 파이프라인, 오케스트레이션 같은 말로 부르기도 해요. 용어는 뒤에서 정리하고, 지금은 감각만 잡아요.

**업무매뉴얼을 그냥 쌓아두는 게 아니라, 실제 작업 순서 안에 배치하는 것.**

Claude Code에서 이 흐름을 만들 때 자주 쓰는 장치는 네 가지예요.
...
이 파일들은 보통 프로젝트 안의 `.claude/` 폴더에 둬요. 폴더 위치와 실제 동작은 본문에서 하나씩 봐요.

## 1. commands는 자주 쓰는 업무 단축키다

`commands`는 `/write`, `/report`, `/script` 같은 명령어예요. 내가 자주 쓰는 업무를 한 줄로 부르는 단축키라고 보면 돼요.
...
예를 들어 `.claude/commands/write.md`를 만들면 Claude Code에서 `/write`처럼 불러 쓸 수 있어요.

여기서 `/write`는 “글만 써라”는 뜻이 아니에요. 이름은 `/write`여도 안쪽에는 전체 흐름을 넣을 수 있어요.
...
이렇게 실제 업무가 흘러가는 방식을 여러 단계로 묶은 걸 **파이프라인 단축키**라고 부르면 돼요.

여기서 정교함이 나와요. 작가가 실제로 어떤 순서로 판단하는지, 어떤 기준으로 소재를 고르는지, 어떤 단계에서 문체를 고치는지 아는 사람이 파이프라인을 잘 만들어요. 그게 도메인 지식이에요.

매번 이렇게 길게 말하지 않기 위해 만드는 거예요.
...
이걸 매번 치면 피곤하잖아요. 그래서 한 줄로 묶어요.

```text
/write 3화
```

이 한 줄이 자동화의 입구예요. 단축키를 실행하면 안쪽에 적어둔 파이프라인대로 일이 시작돼요.

여기서 command의 역할은 단순해요.

> 자주 쓰는 업무 흐름을 한 줄로 부르고, 그 안에서 어떤 목차와 업무 문서를 읽을지 지정하는 것.

예를 들어 `/write 3화` 하나 안에 실제 작가들이 글을 쓰는 업무 흐름을 넣을 수 있어요. 소재를 고르고, 독자 관점을 잡고, 전개 방식을 정하고, 초안을 쓰고, 퇴고하고, 검수하고, 저장하는 흐름까지 한 번에 묶을 수 있어요. 굳이 `/write`와 `/review`를 따로 나눌 필요는 없어요.

## 2. agents는 목차별·역할별 담당자다

담당자를 만드는 이유는 LLM의 특성 때문이에요.

LLM은 한 번에 참고할 수 있는 컨텍스트에 한계가 있어요. 대화가 길어지고 문서가 많아질수록 앞에서 본 기준이 흐려지거나, 지금 작업과 상관없는 기준이 섞일 수 있어요. 이걸 컨텍스트 메모리 한계라고 보면 돼요.
...
각 파일 안에는 “이 담당자는 언제 쓰고, 어떤 기준을 보고, 어떤 결과를 돌려줘야 하는지”를 적어둬요.

- **마스터 에이전트** — 나와 직접 대화하면서 전체 흐름을 잡는 Claude
- **서브 에이전트** — 마스터 에이전트가 특정 역할을 맡겨 부르는 담당자

내가 `/write 3화`를 실행하면 마스터 에이전트가 전체 파이프라인을 잡고, 안쪽에서 서브 에이전트들을 순차적으로 부를 수 있어요.
...
각 서브 에이전트는 자기 단계에 필요한 문서와 기준만 들고 일해요. 그래서 기준이 섞이지 않고 작업이 더 선명해져요.

담당자는 아무렇게나 늘리는 게 아니라, 보통 두 방식으로 나눠요.

### 목차별 담당자

`CLAUDE.md`나 `docs/` 안의 목차가 분명할 때 쓰기 좋아요.

예를 들어 유튜브 대본 프로젝트라면 이런 식이에요.
...
각 담당자는 자기에게 할당된 목차와 문서를 더 강하게 봐요. 그래서 “문서가 있는데도 안 지키는 문제”가 줄어들어요.
...
이 방식은 일이 순서대로 흘러갈 때 편해요. 앞 단계에서 만든 결과를 다음 단계가 이어받아요.
...
핵심은 담당자를 만드는 행위 자체가 아니에요.

**목차와 업무 문서를 실제 처리 단계에 할당하는 것.** 이게 담당자를 쓰는 이유예요.

## 3. skills는 Claude가 상황 보고 꺼내 쓰는 능력이다

`skills`는 내가 매번 직접 부르는 단축키라기보다, Claude가 상황을 보고 꺼내 쓰는 기능에 가까워요.

파일은 보통 폴더 하나에 `SKILL.md` 형태로 둬요.
...
예를 들어 어떤 작업을 하다가 “이건 코드 리뷰 방식으로 봐야겠다”, “이건 보안 검토가 필요하겠다”, “이건 글쓰기 기준을 적용해야겠다”라고 판단하면 그때 맞는 skill을 꺼내는 식이에요.

입문 단계에서 skills를 깊게 만들 필요는 없어요. 지금은 이렇게만 이해하면 돼요.
...
`hooks`는 특정 순간에 자동으로 실행되는 장치예요.

여기서 헷갈리기 쉬운 게 있어요. hooks는 보통 두 부분으로 나눠서 생각하면 돼요.
...
사람이 매번 “검수해”, “기록해”, “포맷 맞춰”라고 말하지 않아도 정해진 순간에 돌아가요.

초반에는 hooks까지 욕심낼 필요 없어요. 먼저 commands와 agents로 흐름을 잡고, 반복되는 검수나 정리 작업이 보이면 그때 hooks로 넘기면 돼요.

## 5. 네 가지는 한 흐름으로 묶인다

자동화는 네 기능을 따로 외우는 공부가 아니에요. 한 작업 안에서 어떻게 이어지는지 보면 돼요.

예를 들어 `/write 3화`라는 파이프라인 단축키는 이렇게 돌아갈 수 있어요.

이때 파일 위치로 보면 대략 이런 조합이에요.
...
마지막에 `proofreader`가 기준을 통과시키지 못하면 `writer`로 다시 돌아가는 흐름까지 묶을 수 있어요.

도입부에서 미뤄둔 용어를 여기서 정리하면 이래요.

- **워크플로우** — 일이 흘러가는 전체 순서
- **파이프라인** — 앞 단계 결과가 다음 단계로 넘어가는 작업 라인
- **오케스트레이션** — 여러 역할과 문서를 조율해서 끝까지 처리하는 것

말은 달라도 감각은 하나예요. 이 흐름을 어떤 이름으로 불러도 돼요.

여기서 잡아야 할 감각은 이거예요.

**어떤 파이프라인 단축키를 실행하면 어떤 목차와 문서를 읽고, 실제 업무를 어떤 순서와 기준으로 처리할지 정해두는 것.** 이게 자동화의 시작이에요.

## 자주 헷갈리는 포인트

### 자동화하면 완전히 손 놓아도 되나요?

처음엔 아니에요. 결과물을 읽고 내 의도와 맞는지 계속 확인해야 해요.

자동화 자체를 만드는 건 의외로 쉬워요. 어려운 건 **내가 원하는 결과물대로 나오게 만드는 것**이에요. 파이프라인 단축키를 만들고, 서브 에이전트를 나누고, 문서를 연결하는 건 금방 할 수 있어요. 진짜 시간은 그다음부터 들어가요.

나는 최소 2주 정도는 결과물을 보면서 품질 개선 작업이 필요하다고 봐요.
...
이 루프를 돌면서 자동화가 내 일에 맞아져요.

결국 사람의 만족도나 취향은 디테일 차이에요. 내가 원하는 결과물이 나오느냐는 내가 얼마나 의도를 잘 설명했고, 판단 기준을 얼마나 구체적으로 설계했느냐에 달려 있어요. 자동화는 처음부터 완성품이 아니라, 돌리고 읽고 고치면서 품질을 맞춰가는 작업 환경이에요.

## 오늘 바로 해볼 것

오늘은 실제로 자동화를 만들기보다, 내가 현업에서 하고 있는 업무를 하나 떠올려서 워크플로우로 설계해봐요.
...
여기서는 설계만 해보면 돼요. 실제로 `.claude/commands/`, `.claude/agents/`, `.claude/skills/`, `.claude/settings.json`에 파일을 만들고 실행하는 건 다음 글에서 해볼 거예요.

당장 뭘 해야 할지 모르겠다면 괜찮아요. 다음 글에서 블로그 자동화 예시로 같이 해보면 돼요.
