...
- [x] **WorkspaceDocs blog 분기 toss 마크업 리스킨 (v1, 2026-05-14)** — `WorkspaceDocs.tsx` 의 `if (isBlog)` 분기만 `PublicBlogHome` 의 `.pbh-*` 마크업/CSS 로 교체. 별도 컴포넌트 신설 없이 `PublicBlogHome.css` 를 side-by-side import 해 재사용. 데이터 매핑: 기존 `hero/popularList/blogCategories/rest` → `.pbh-hero` 카드 + `.pbh-main-grid` (전체 아티클 좌측 + 인기 글 우측 sidebar) + 카테고리 그리드. team-blog 분기·promoted/endorsed rail 미변경. **docs/wiki 분기는 이전 `.blog-*` 마크업 그대로 유지** (사용자 결정 2026-05-14: 같은 default 분기가 모든 type 영향 가는 걸 인지하고 분기 분리 선택, type 차이가 마크업 차이로 가도 노브 collapse 결정과 호환 — type 자체가 노브). → **metric: :3600 blog → `.pbh-*` (마커 1), :3601/3602 docs → `.blog-*` (마커 1), 상호 0 으로 분리 확인됨**
...
- core/packages/viewer/src/pages/workspace/WorkspaceDocs.tsx — `if (isBlog)` 게이트 추가, blog 분기만 `.pbh-*` toss 마크업, docs/wiki/기타 분기는 이전 `.blog-*` 마크업 (blog-hero / blog-sub-featured / blog-categories / blog-articles) 그대로 복원. team-blog 분기·promoted/endorsed rail 미변경. `subFeatured = sorted.slice(1,3)` 변수 유지 (docs 분기에서 사용). `PublicBlogHome.css` side-by-side import (blog 분기 전용). (2026-05-14, 분기 분리 결정: docs 영향 분리)
- core/packages/viewer/src/pages/PublicBlogHome.css — toss 토큰 정밀화 추가 (미디어 12px radius, title 1.25rem/1.45/tracking-title, 본문 17px, tag pill 8px/13px) — `/`, `/feed`, per-workspace blog 랜딩 공통 (2026-05-14)
