...
- [planned] **iter 17+**: searchWrapper width 격차 (SPA 420px desktop / SSG 296~356px — `.ssg-header-center max-width: 480px` + flex 부모 grid 차이) + mobile 처리 (SPA 는 `max-width: 180px` 로 축소, SSG 는 `display: none` 으로 hide). footer / 본문 container padding 등 잔여 wedge 도 순차 진행.
...
- [ ] iter 17: searchWrapper width 격차 — SPA 420px (desktop/tablet, grid `minmax(200px,420px)`) vs SSG 296~356px (`max-width: 480px` 인데 flex 부모가 못 펴줌). SSG `.ssg-header-center` 가 grid `minmax(200px, 420px)` 형태로 변경되거나, `width: 420px` 직접 지정 필요. mobile: SPA `max-width: 180px` 로 축소 (icon-only 형태) vs SSG `display: none` (line 695-698) — SSG 도 hide 대신 shrink 로 변경.
- [ ] iter 18: footer 격차 — bg color SPA transparent vs SSG white/dark, padding SPA `20px 0px 0px` vs SSG `0px`, width SPA 840 (column 안) vs SSG 1016 (full). SSG footer 가 main column 안으로 들어가야 함.
- [ ] iter 19: tablet/mobile 본문 container 좌우 padding — mobile docTitle.x SPA 16 vs SSG 32, tablet SPA 80 vs SSG 92. 12~16px 일정한 격차로 SSG 가 더 안쪽. blog-detail 또는 main padding 차이 추정.
