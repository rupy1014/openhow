---
slug: source-map
title: "소스 매핑"
nav: 소스 매핑
status: draft
---

# 챕터별 참고 소스 매핑

> 기준 목차: [toc-final](./toc-final)

## Part 1. Codex로 자동화 시작하기

### 01장 한 줄 설치, 바로 실습
- `examples/clauders.ai/docs/입문/01-설치.md` — 설치 흐름과 최소 준비물
- `examples/clauders.ai/docs/입문/02-프로젝트설정.md` — 프로젝트 시작 장면
- `examples/clauders.ai/docs/입문/03-hello-world.md` — 첫 실행 실습 감각
- `examples/clauders.ai/docs/바이브코딩연재/07-vs-code-세팅.md` — VS Code 세팅 보조
- `examples/clauders.ai/docs/바이브코딩연재/09-환경-한방에-세팅하기.md` — “마법의 한 줄” 톤 참고

### 02장 자동화 감 잡기
- `examples/clauders.ai/docs/자동화/00-4가지기능.md` — 자동화 큰 그림
- `examples/clauders.ai/docs/자동화/01-작업맡기기.md` — 한 단계씩 맡기는 감각
- `examples/clauders.ai/docs/자동화/02-멀티에이전트.md` — 워크플로우/서브에이전트 설명용
- `examples/clauders.ai/docs/자동화/05-브라우저자동화.md` — 블로그 발행 자동화 참고
- `examples/clauders.ai/docs/AI로일하기/00-AI로일하기.md` — 실무 자동화 감각

### 03장 완전 초보자를 위한 설명
- `examples/clauders.ai/docs/완전초보/01-AI기초.md` — AI/모델/회사 구분
- `examples/clauders.ai/docs/완전초보/03-개발환경.md` — GUI/CLI/환경 개념
- `examples/clauders.ai/docs/완전초보/04-컨텍스트.md` — 컨텍스트와 메모리
- `examples/clauders.ai/docs/완전초보/05-계획모드.md` — 계획 모드 감각
- `examples/clauders.ai/docs/레시피/03-codex-orchestration.md` — 오케스트레이션/설정 예시

### 04장 웹소설 자동화
- `/Users/taesupyoon/sideProjects/web-novel/docs/웹소설_작가되기_웹소설_장르.md` — 연재 생태계와 독자 기대치
- `/Users/taesupyoon/sideProjects/web-novel/docs/웹소설_PD가_알려주는_플랫폼별_특징.md` — 플랫폼/연재 맥락
- `/Users/taesupyoon/sideProjects/web-novel/docs/웹소설_1화_작성하는_방법_독자를_사로잡는_5가지_공식_1.md` — 초반 회차 설계
- `/Users/taesupyoon/sideProjects/web-novel/docs/술술_읽히는_문장_실전_웹소설_퇴고하는_법.md` — 퇴고와 품질 개선
- `examples/clauders.ai/docs/부록/03-1-write-scene.md` — 장면 단위 작성 파이프라인

## Part 2. 기초 다지기 — 바이브코딩 입문

### 05장 AI 캐릭터 채팅 서비스 빠르게 만들어보기
- `examples/clauders.ai/docs/바이브코딩/02-바이브기획.md` — 기획 흐름
- `examples/clauders.ai/docs/바이브코딩/02-1-PRD작성법.md` — plan 모드/요구사항 정리
- `examples/clauders.ai/docs/바이브코딩/03-레퍼런스.md` — 레퍼런스 주기의 중요성
- `examples/clauders.ai/docs/바이브코딩연재/37-AI-챗봇-만들기.md` — 캐릭터 채팅 서비스 뼈대
- `examples/clauders.ai/docs/바이브코딩연재/16-기획과-아키텍처.md` — 기획~개발 흐름 연결

### 06장 만든 서비스를 직접 배포해보기
- `examples/clauders.ai/docs/바이브코딩연재/08-git-기초.md` — Git/GitHub 입문
- `examples/clauders.ai/docs/바이브코딩/06-배포.md` — 배포 흐름
- `examples/clauders.ai/docs/바이브코딩연재/19-배포하기.md` — 링크로 공개되는 감각
- `examples/clauders.ai/docs/입문/05-비용관리.md` — 배포 비용/운영 감각

### 07장 빌더로 천천히 성장하기
- `examples/clauders.ai/docs/바이브코딩/02-2-안만들것.md` — MVP 범위 자르기
- `examples/clauders.ai/docs/바이브코딩/07-피드백.md` — 빠른 피드백 루프
- `examples/clauders.ai/docs/바이브코딩/08-5-안티패턴.md` — 흔한 함정 정리
- `examples/clauders.ai/docs/바이브코딩연재/16-기획과-아키텍처.md` — 스코프/기획 언어 보강

## Part 3. 자동화 구조 읽기

### 08장 자동화 시스템은 어떻게 구성될까
- `examples/clauders.ai/docs/AI회사/00-AI회사.md` — 큰 그림
- `examples/clauders.ai/docs/AI회사/03-멀티-에이전트는-어떻게-짜.md` — 역할 구조
- `examples/clauders.ai/docs/레시피/03-codex-orchestration.md` — 오케스트레이션 관점
- `examples/clauders.ai/docs/웨비나/openclaw-to-kakaotalk.md` — OpenClaw 사례

### 09장 완전 자동화는 왜 아직 어려운가
- `examples/clauders.ai/docs/AI회사/06-막히면-어디부터-봐.md` — 장애/한계 지점
- `examples/clauders.ai/docs/AI회사/90-팩트체크와-개선.md` — 정확도와 검증
- `examples/clauders.ai/docs/자동화/00-4가지기능.md` — 구조 대비 한계 설명
- `examples/clauders.ai/docs/바이브코딩/08-한계와다음.md` — 과장 방지용 관점

### 10장 AI가 팀처럼 일한다는 건 무슨 뜻일까?
- `examples/clauders.ai/docs/AI회사/01-Paperclip은-뭐야.md` — 사례 소개
- `examples/clauders.ai/docs/AI회사/03-멀티-에이전트는-어떻게-짜.md` — 역할 분담
- `examples/clauders.ai/docs/AI회사/04-할일과-heartbeat은-어떻게-굴러.md` — 운영 루프
- `examples/clauders.ai/docs/AI회사/05-승인과-비용은-어떻게-봐.md` — 승인/비용/기록

## Part 4. 실전 자동화 유즈케이스

### 11장 쓰레드 자동화와 뉴스봇 만들기
- `examples/clauders.ai/docs/AI로일하기/00-AI로일하기.md` — 실무 맥락
- `examples/clauders.ai/docs/자동화/03-사례모음.md` — 사례 확장
- `examples/clauders.ai/docs/바이브코딩연재/25-자동화-아이디어.md` — 아이디어 발상
- `examples/clauders.ai/docs/바이브코딩연재/28-정기-실행.md` — 정기 실행/봇화

### 12장 유튜브 자동화 워크플로우 만들기
- `examples/clauders.ai/docs/부록/03-remotion-video-pipeline.md` — 영상 파이프라인
- `examples/clauders.ai/docs/부록/03-2-tts-render.md` — TTS/렌더링 단계
- `examples/clauders.ai/docs/부록/06-영상초안-1화-이력서데모.md` — 실제 산출물 흐름
- `examples/clauders.ai/_drafts/유튜브/*` — 필요시 사례 확장

### 13장 문서와 발표자료 자동화
- `examples/clauders.ai/docs/AI로일하기/04-바이브PPT.md` — 발표자료 자동화
- `examples/clauders.ai/docs/AI로일하기/05-바이브제안서.md` — 제안서 자동화
- `examples/clauders.ai/docs/AI로일하기/07-프로젝트브레인.md` — 문서 축적/재사용

### 14장 내가 일하는 곳에 자동화 적용하기
- `examples/clauders.ai/docs/AI로일하기/01-내업무에적용하기.md` — 업무 적용 관점
- `examples/clauders.ai/docs/자동화/03-사례모음.md` — 업종 확장
- `examples/clauders.ai/docs/AI로일하기/08-AI서비스-원가분석.md` — 현실성/비용 감각
