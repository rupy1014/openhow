---
slug: automation/03-patterns
title: 사례 모음
nav: 사례 모음
description: '체인, 분류, 동시처리, 지휘자, 반복개선 패턴을 실제 업무 예시로 정리해 내 자동화 흐름을 설계할 때 참고하게 한다.'
thumbnail: /__content__/images/img-AT03.png
hook: '원리는 같지만, 응용은 다양하게'
status: 출시
tags: [automation, patterns, chain, routing, parallel, orchestrator, evaluator]
---

## 핵심 요약

**자동화 패턴은 목차와 담당자를 어떤 순서로 움직일지 정하는 설계도야.**
...
자기소개서를 쓴다면 경력 분석이 먼저고, 그다음 스토리 작성, 문체 정리, 적합성 검수로 간다.
...
분류는 먼저 내용을 보고 어디로 보낼지 정하는 방식이야.
...
`router`는 이 문서를 읽고 어느 담당자에게 보낼지 정한다.

## 3. 동시처리 — 서로 기다릴 필요가 없는 일

동시처리는 여러 담당자가 동시에 움직여도 되는 방식이야.
...
경쟁사 조사는 서로 기다릴 필요가 없다. 각자 조사한 뒤 `analyst`가 합치면 된다.
...
마지막에는 통합 담당자가 필요하다. 동시에 나온 결과를 한곳에 모아야 하기 때문이야.

## 4. 지휘자 — 상황을 보고 필요한 담당자를 부르는 일

지휘자는 미리 정해진 순서대로만 가지 않는다. 결과를 보며 다음 담당자를 고른다.
...
사업 기획서에 법무 이슈가 없으면 `legal-reviewer`를 부르지 않아도 된다. 리스크가 크면 `risk-reviewer`에게 깊게 보게 할 수 있다.

지휘자 흐름은 예외가 많은 작업에 어울린다. 초반에는 체인으로 시작하고, 갈림길이 많아질 때 지휘자 흐름을 붙이면 된다.

## 5. 반복개선 — 만들고, 평가하고, 고치는 일

반복개선은 초안을 만들고 평가한 뒤, 기준에 못 미치면 다시 고치는 방식이야.
...
멈추는 조건이 있어야 한다. 그래야 반복개선이 무한히 돌지 않는다.

## 6. 패턴은 다섯이지만 원리는 같다

다섯 가지 패턴을 따로 외울 필요는 없어. 사실 워크플로우의 기본 원리는 다 비슷해.

> 어떤 입력을 받아서, 누가 어떤 순서로 처리하고, 어떤 기준으로 결과를 확인할지 정한다.

체인이든 분류든 동시처리든 지휘자든 반복개선이든, 이 뼈대는 똑같아. 차이는 어떻게 응용하느냐에서 나온다.
...
같은 뼈대인데 적용 지점이 다를 뿐이야. 그래서 실제 업무에서는 한 패턴만 쓰지 않아.
...
**패턴 이름을 외우는 게 아니라, 내 업무 안에서 어디에 어떤 응용이 필요한지 보는 것.** 같은 사람이 만들어도 도메인이 다르면 응용 방식이 달라진다. 마케팅 담당자의 분류 흐름과 CS 담당자의 분류 흐름은 분기 기준부터 다르다. 원리는 같고, 응용이 본인 업무 지식에서 갈린다.
...
아니야. 먼저 내 업무 흐름을 적고, 그 흐름에 맞는 이름을 붙이면 된다.

### 복잡하게 만들수록 자동화가 잘 돼?

아니야. 한 번에 되는 일은 한 번에 시키면 된다. 반복되는 문제가 보일 때 흐름을 나누면 된다.

### 언제 담당자를 추가해?

결과를 읽었을 때 같은 문제가 반복되면 추가한다. 문체가 흔들리면 문체 담당자, 검수가 약하면 검수 담당자, 자료 누락이 반복되면 자료 확인 담당자를 만든다.

## 오늘 바로 해볼 것

내가 자동화하고 싶은 일 하나를 골라서 아래처럼 적어봐.
