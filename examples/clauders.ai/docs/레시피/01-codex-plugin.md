...
사용자 → "결제 취소 API 만들어줘"
              ↓
Phase 1  Claude: 코드 분석 + 핵심 스니펫 수집 + 작업 분해
              ↓ (사용자 확인)
Phase 2  Claude → Codex: 5요소 프롬프트 위임 (TASK/EXPECTED/MUST NOT/CONTEXT/VERIFY)
         Codex: 구현 + 자체 검증(VERIFY)
         Claude: git diff 확인, 중복/누락/품질 검증
         ↳ 이슈 발견 → Codex에 수정 재위임 (최대 3회)
         ↳ 3스텝 이상이면 /compact로 컨텍스트 정리
              ↓ (스텝마다 반복, 다음 스텝 파일은 미리 분석)
Phase 3  Claude: Codex 리뷰 → 이슈 있으면 수정 위임 → 코드 완료 보고
              ↓ (사용자는 여기서 해방)
Phase 4  (백그라운드) 관련 docs/md 업데이트 + 조건부 대청소
```

**3가지 핵심이야.**

첫째, **VERIFY**. 위임할 때 5번째 요소로 검증 커맨드를 넣어. Codex가 코드 짜고 돌려주기 전에 스스로 테스트를 돌려. 퇴고 루프가 줄어들어.

```
TASK: 결제 취소 API 추가
EXPECTED: cancel() 호출 시 status → cancelled
MUST NOT: 다른 모델 파일 수정 금지
CONTEXT: [관련 코드 10~20줄 직접 포함 — 경로만 넘기면 Codex가 파일 읽느라 토큰 낭비]
VERIFY: npm test -- --grep "order cancel" 실행, pass/fail 결과 포함해서 돌려줘
```

둘째, **파이프라이닝**. Codex가 현재 스텝을 실행하는 동안 Claude가 다음 스텝에 필요한 파일을 미리 읽어둬. 직렬로 하면 느린데, 이러면 벽시계 시간이 줄어.

셋째, **문서는 백그라운드**. 코드 완료 보고를 사용자에게 먼저 하고, 문서 정리는 뒤에서 돌아. 기다릴 필요 없어. 관련 md를 찾아서 기존 파일에 반영하고, 1,000줄 넘으면 분할하고, 비슷한 주제가 3개 이상이면 폴더로 묶어. docs 변경이 쌓이면 대청소도 자동으로 트리거돼.
...
| 위임 프롬프트 | 사용자 메시지 그대로 | 5요소 구조 (VERIFY 포함) |
| 검증 | 없음 | Codex 자체검증 + Claude git diff + Read |
| 퇴고 | 없음 | 이슈 → 재위임 (최대 3회) |
| 문서 반영 | 없음 | 백그라운드로 docs/md 자동 업데이트 |
| 토큰 최적화 | 없음 | 스니펫 주입, /compact, 파이프라이닝 |
