...
### 의도 중심으로 돌리기 — /intent + /cowork

이 오케스트레이션을 **의도(Intent) 기반으로 자동화**할 수 있어. `/intent`로 왜 만드는지 구조화하고, `/cowork`로 실행하면 의도가 자동으로 로드돼서 TASK/MUST NOT/CONTEXT가 알아서 채워져.

자세한 건 [Intent Engineering — 의도 중심 개발 세팅](./06-intent-cowork-린스타트업.md)을 봐.
