...
> "AI한테 뭔가 물어보고 답 받는 건 어떻게 해?"

API라는 건 **남의 서버에 데이터를 요청하고, 응답을 받는 구조**야.
환율 사이트에 "달러 환율 알려줘" 하면 숫자가 오듯이,
...
이건 절대 코드에 직접 적으면 안 돼. 04편에서 배운 `.env` 파일에 넣어야 해.
...
`fetch`로 요청 보내고, `await`로 기다리고, `try-catch`로 에러 방지.
이 세 가지 패턴은 앞으로 외부 서비스 연결할 때 계속 반복돼.
...
AI API는 일반 API랑 구조가 같아. `fetch`로 메시지를 보내고 답변을 받는 것뿐이야. system prompt만 잘 쓰면 요약, 번역, 채팅 뭐든 돼.
