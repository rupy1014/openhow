...
**I. 요구사항 명세** (SRS)
1. [기능 요구사항 (Functional Requirements)](#1-기능-요구사항)
2. [비기능 요구사항 (Non-Functional Requirements)](#2-비기능-요구사항)
3. [시스템 제약사항 (Constraints)](#3-시스템-제약사항)

**II. 시스템 아키텍처** (SAD)
4. [솔루션 개요 (Solution Overview)](#4-솔루션-개요)
5. [기술 스택 (Technology Stack)](#5-기술-스택)
6. [시스템 아키텍처 설계 (System Architecture Design)](#6-시스템-아키텍처-설계)
    ...
    def _extract_features(self, orders):
        ...
        for order in orders:
            feature_vector = [
                ...
                order['producer_rating'],  # 0-5
                order['producer_transaction_count'],
                order['hour_of_day'],  # 0-23
                order['day_of_week'],  # 0-6
            ]
...
# 주간 학습 (Sunday 00:00)
detector.train(get_orders_by_period('2026-01-05', '2026-01-12'))

# 실시간 탐지
new_order = {
    'quantity': 10000,  # 비정상적으로 높음
    'price': 100,
    'delivery_time_hours': 48,
    'producer_rating': 2.1,  # 낮은 평점
    'producer_transaction_count': 5,  # 거래 적음
    'hour_of_day': 3,  # 새벽
    'day_of_week': 2  # 화요일
}

result = detector.predict(new_order)
if result['is_anomaly']:
    # 관리자 알림
    alert_admin(f"[이상거래] {result['score']:.2f} → 거래 보류")
    # 거래 대기열에 이동
    move_to_hold_queue(new_order)
```
...
CREATE TABLE producers (
    ...
    farm_coordinates POINT NOT NULL,  -- PostGIS
...
이 기술명세서 v2.0은 **업계 표준 SRS + SAD 형식**으로 재구성되어, 다음을 제공합니다:
